...
public class GGame {
	...
	public GGame() {
		...
		device.setDisplayMode(new DisplayMode(GameConstants.Game_Width, GameConstants.Game_Height, 32, 0));
	...
	public static void DecreaseLive() {
		if (currentPlayer.getLives() == 1)
			...
			((mytank) Entities[1]).Reset(); // Reset Bar to first status
		...
		while (true) {
			...
			if (Levelstatus == LevelStatus.LevelRunning) {
				for (int i = 0; i < cEntities; i++)
					Entities[i].Step();
				CheckCollisions();
				currentPlayer.setCurrentTime((System.currentTimeMillis() - currentPlayer.getStartTime()) / 1000);
			}
	...
	private class KeyHandler extends KeyAdapter {
		...
		public void keyReleased(KeyEvent e) {
			...
			if (e.getKeyCode() == KeyEvent.VK_CONTROL) {
				...
				if (Levelstatus == LevelStatus.LevelNotStarted || Levelstatus == LevelStatus.LevelStop_LiveLoss
						|| Levelstatus == LevelStatus.LevelStop_Player)
					Levelstatus = LevelStatus.LevelRunning;
				return;
			}

			if ((Levelstatus == LevelStatus.LevelRunning) || (Levelstatus == LevelStatus.LevelStop_LiveLoss)) {
